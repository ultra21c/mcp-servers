...
const TOTAL_SEARCH_TOOL = {
    name: "daum_total_search",
    description: "다음 통합검색 서비스에서 질의어로 웹, 블로그, 카페, 동영상, 이미지, 책 정보를 동시에 검색합니다. " +
...
// 타입 가드 함수들
function isDaumSearchArgs(args) {
...
async function performWebSearch(query, sort, page, size) {
    const data = await performSearch('web', { query, sort, page, size });
    return data.documents.map((doc) => `제목: ${doc.title}\n내용: ${doc.contents}\nURL: ${doc.url}\n작성시간: ${doc.datetime}`).join('\n\n');
}
async function performVclipSearch(query, sort, page, size) {
    const data = await performSearch('vclip', { query, sort, page, size });
    return data.documents.map((doc) => `제목: ${doc.title}\n재생시간: ${doc.play_time}초\n썸네일: ${doc.thumbnail}\nURL: ${doc.url}\n작성자: ${doc.author}\n작성시간: ${doc.datetime}`).join('\n\n');
}
async function performImageSearch(query, sort, page, size) {
    const data = await performSearch('image', { query, sort, page, size });
    return data.documents.map((doc) => `컬렉션: ${doc.collection}\n썸네일: ${doc.thumbnail_url}\n이미지: ${doc.image_url}\n출처: ${doc.display_sitename}\nURL: ${doc.doc_url}\n작성시간: ${doc.datetime}`).join('\n\n');
}
async function performBlogSearch(query, sort, page, size) {
    const data = await performSearch('blog', { query, sort, page, size });
    return data.documents.map((doc) => `제목: ${doc.title}\n내용: ${doc.contents}\n블로그명: ${doc.blogname}\nURL: ${doc.url}\n썸네일: ${doc.thumbnail}\n작성시간: ${doc.datetime}`).join('\n\n');
}
async function performBookSearch(query, sort, target, page, size) {
...
async function performCafeSearch(query, sort, page, size) {
    const data = await performSearch('cafe', { query, sort, page, size });
    return data.documents.map((doc) => `제목: ${doc.title}\n내용: ${doc.contents}\n카페명: ${doc.cafename}\nURL: ${doc.url}\n썸네일: ${doc.thumbnail}\n작성시간: ${doc.datetime}`).join('\n\n');
}
async function performTotalSearch(query, size = 10) {
